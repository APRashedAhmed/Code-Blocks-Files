//Linear Algebra Operators Program
//Program works only on square matrices (for now)
...
//Make sure to set the correct number of dimensions on the matrices
int const n=3;
...
void MultMat(double A[n], double B[n], double AB)
{
    for(int i=0; i<n; i++)
        AB = AB + A[i]*B[i];
}

void MultMat(double A, double B[n], double AB[n])
{
    for(int i=0; i<n; i++)
        AB[i] = A*B[i];
